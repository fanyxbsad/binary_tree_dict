...
from typing import Callable, Any, List, Tuple, Iterator as IterType

def _key_rank(k: Any) -> int:
    if k is None: return 0
    if isinstance(k, (int, float)): return 1
    if isinstance(k, str): return 2
    return 3
...
class BinaryTreeDict:
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return sorted(to_list(self), key=_safe_sort_key) == sorted(to_list(other), key=_safe_sort_key)
...
def cons(key: Any, value: Any, d: BinaryTreeDict) -> BinaryTreeDict:
    def _insert(node: _Node) -> _Node:
        ...
        if _lt(key, node.key):
            ...
            if new_left is node.left: return node
            return _Node(node.key, node.value, new_left, node.right)
        else:
            new_right = _insert(node.right)
            if new_right is node.right: return node
            return _Node(node.key, node.value, node.left, new_right)
...
def member(key: Any, d: BinaryTreeDict) -> bool:
    def _mem(node: _Node) -> bool:
        ...
        if _lt(key, node.key): return _mem(node.left)
        return _mem(node.right)
    return _mem(d._root)

def remove(d: BinaryTreeDict, key: Any) -> BinaryTreeDict:
    def _del(node: _Node) -> _Node:
        ...
        if _lt(node.key, key):
            ...
            if new_right is node.right: return node
            return _Node(node.key, node.value, node.left, node.right)
...
def from_list(lst: List[Tuple[Any, Any]]) -> BinaryTreeDict:
    def _from(idx: int, acc: BinaryTreeDict) -> BinaryTreeDict:
        if idx == len(lst): return acc
        k, v = lst[idx]
        return _from(idx + 1, cons(k, v, acc))
    return _from(0, empty())

def concat(d1: BinaryTreeDict, d2: BinaryTreeDict) -> BinaryTreeDict:
    lst2 = to_list(d2)
    def _concat(idx: int, acc: BinaryTreeDict) -> BinaryTreeDict:
        if idx == len(lst2): return acc
        k, v = lst2[idx]
...
def find(d: BinaryTreeDict, predicate: Callable[[Any, Any], bool]) -> Tuple[Any, Any]:
...
def filter_lst(d: BinaryTreeDict, predicate: Callable[[Any, Any], bool]) -> BinaryTreeDict:
    return from_list(list(filter(lambda t: predicate(t[0], t[1]), to_list(d))))

def map_lst(d: BinaryTreeDict, func: Callable[[Any, Any], Any]) -> BinaryTreeDict:
    return from_list(list(map(lambda t: (t[0], func(t[0], t[1])), to_list(d))))

def reduce_lst(d: BinaryTreeDict, func: Callable[[Any, Any, Any], Any], initial: Any) -> Any:
    return reduce(lambda acc, t: func(acc, t[0], t[1]), to_list(d), initial)
